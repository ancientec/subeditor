    ...
    const fetchPage = (page, keyword) => {
        ...
        cfg["image.library.fetch"](page, cfg["image.library.per.page"], keyword, (total, data) => {
            ...
            if (cfg["image.library.allow.paging"]) {
                ...
                prev.innerHTML = '<span class="se-icon">' + subeditor_1.default.svgList['previous'] + '</span>';
                ...
                next.innerHTML = '<span class="se-icon">' + subeditor_1.default.svgList['next'] + '</span>';
            ...
            if (cfg["image.library.allow.search"]) {
                ...
                search.innerHTML = '<span class="se-icon">' + subeditor_1.default.svgList['search'] + '</span>';
...
const processUpload = (editor, idx, file, el, completed) => {
...
const renderUpload = (editor, content) => {
    var _a;
    content.classList.remove("horizontal");
        ...
        Array.from(files).forEach(file => {
            ...
            processUpload(editor, count - 1, file, tile, (status) => {
                ...
                if (!status)
                    failed++;
                if (completed >= count) {
                    (_a = content.querySelector(".FileTileImageGrid button.upload")) === null || _a === void 0 ? void 0 : _a.remove();
                    //end of upload
                    if (failed > 0 && completed === failed) {
                        ...
                        if (cfgUploadAcceptFiles === 0 || count < cfgUploadAcceptFiles) {
                            ...
                            dropBtnTile.innerHTML = subeditor_1.default.svgList["plus"];
...
const renderURL = (editor, content) => {
    var _a;
    content.classList.remove("horizontal");
    content.innerHTML = '';
...
exports.default = [
    {
        ...
        target: [],
        callback: () => {
            return { "test": subeditor_1.default.svgList["b"] };
        }
    },
    {
        event: "registerCss",
    ...
    },
    {
        ...
        callback: (editor) => {
            return {
                library: {
                    command: "library",
                    svg: subeditor_1.default.svgList["image_library"],
    ...
    },
    {
        ...
        callback: (editor) => {
            ...
            return {
                image: {
                    command: "image",
                    svg: subeditor_1.default.svgList["image"],
                            ...
                            if (!menu.classList.contains("is-active")) {
                                ...
                                if (cfgFeatures.length === 1 && cfgFeatures[0] === "library") {
                                    ...
                                    cfgFeatures.forEach((f) => {
                                        switch (f) {
                                            case 'url':
                                                editor.dom.appendString2Node('<span class="se-button se-ToolbarItem" data-command="' + f + '" data-value="' + f + '" data-tips="image ' + f + '"><span class="se-icon">' + subeditor_1.default.svgList["link"] + '</span></span>', content);
                                            ...
                                            case 'upload':
                                                if (!cfg["image.upload.url"])
                                                    return;
                                                editor.dom.appendString2Node('<span class="se-button se-ToolbarItem" data-command="' + f + '" data-value="' + f + '" data-tips="image ' + f + '"><span class="se-icon">' + subeditor_1.default.svgList["upload"] + '</span></span>', content);
                                            ...
                                            case 'library':
                                                if (typeof cfg["image.library.fetch"] !== "function")
                                                    return;
                                                editor.dom.appendString2Node('<span class="se-button se-ToolbarItem" data-command="' + f + '" data-value="' + f + '" data-tips="image ' + f + '"><span class="se-icon">' + subeditor_1.default.svgList["image_library"] + '</span></span>', content);
    ...
    },
    {
        ...
        callback: (editor, type, action, value) => {
            ...
            if (editor.refContent.lastChild === nodes[0]) {
                if (((_b = (_a = editor.refContent.lastElementChild) === null || _a === void 0 ? void 0 : _a.previousElementSibling) === null || _b === void 0 ? void 0 : _b.nodeName) === "P") {
